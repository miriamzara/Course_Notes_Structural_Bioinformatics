...
\item {How are loops modelled? \done}
...
\paragraph{How are loops modelled?}
Loops are typically the most variable regions. They can be modelled using either database methods (searching for similar loops in PDB) or with ab-initio methods (sampling conformations and scoring them). The major limitation of database
methods is that not always there are loops of the right length and sequence in the database and that satisfy the geometrical constraints of raw model. Instead the problem of ab-initio methods is the computing time.



\paragraph{How are side chains modelled?}
Side chains are usually modelled after the backbone has been constructed. It is important to get them right, because even if the RMSD of the backbone is low, the precise positioning of side chains can be critical for the function of the protein, especially in active sites or binding pockets.
Their conformations are mainly determined by rotations around single bonds, described by the side-chain torsion angles $\chi$. These angles do not take arbitrary values, but tend to adopt a small number of preferred conformations called \textbf{rotamers}. For each torsion angle, the main preferred positions are:
